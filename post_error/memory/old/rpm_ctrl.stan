...
functions {
  vector get_theta(int N, vector time, int R_mu, int R_sigma, int R_beta, matrix x_mu, matrix x_sigma, matrix x_beta, matrix x_alpha, matrix delta_mu,  matrix delta_sigma, matrix delta_beta, vector delta_alpha) {
    ...
    for (n in 1:N) {
      ...
      phi[1] = normal_cdf(time[n], mu[n, 1], sigma[n, 1]); // prob. that response is prepared
      neg_phi[1] = 1 - phi[1];

      psi[1] = neg_phi[1] * alpha[n]; 
      psi[2] = phi[1]  * beta[n, 1];
...
transformed parameters {
  vector[N] theta;
  theta = get_theta(N, time, R_mu, R_sigma, R_beta, x_mu, x_sigma, x_beta, x_alpha, delta_mu, delta_sigma, delta_beta, delta_alpha);
}
model {

  // informative priors for intercepts, to improve identifiability
  delta_mu[1, 1] ~ normal(-1, .25);
  delta_sigma[1, 1] ~ normal(-1, .25);
  delta_beta[1, 1] ~ normal(2, .25);
  delta_alpha[1] ~ normal(-1, .25);
