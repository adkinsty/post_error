...
model {
  ...
  mu_mean ~ normal(-1, .1);
  sigma_mean ~ normal(-2, .1);
  beta_mean ~ normal(2, .1);
  ...
  alpha0 ~ normal(alpha_mean, alpha_sd);

  // priors on group-level means and sds of slopes
  delta_mu_mean ~ normal(0, .1);
  delta_sigma_mean ~ normal(0, .1);
  delta_mu_sd ~ normal(.5, .1);
  delta_sigma_sd ~ normal(.5, .1);

  // priors on id-level slopes
  delta_mu ~ normal(delta_mu_mean, delta_mu_sd);
  delta_sigma ~ normal(delta_sigma_mean, delta_sigma_sd);
