...
mod_name <- 'rpm1_hier_mu_err0_sigma_err0_beta_err0'
...
get_mu(draws, R_mu = 1, covar = TRUE) %>% spread_draws(mu[v]) %>% median_qi()
get_sigma(draws, R_sigma = 1, covar = TRUE) %>% spread_draws(sigma[v]) %>% median_qi()

plt <- plot_prep1_cov(draws, exp_name)

delta_mu <- extract(fit, 'delta_mu_loc') %>% unlist() 
  delta_mu %>% p_direction() * sign(median(delta_mu))

delta_sigma <- extract(fit, 'delta_sigma_loc') %>% unlist() 
...
plt_dat <- dat %>% 
  ...
  smooth_obs(covar = c('err0')) %>% mutate(err0 = covar1)
plt_dat %>% 
...
plt_dat <- dat %>% 
  mutate(X = PT, Y = accuracy) %>%
  smooth_obs(covar = c('err0', 'err00')) %>% mutate(err0 = covar1, err00 = covar2)
plt_dat %>% 
  ggplot(aes(x = PT, y = obs_mu, 
    ymax = obs_mu + obs_se * 1.96, ymin = obs_mu - obs_se * 1.96, 
...
plt_dat <- dat %>% 
  mutate(X = PT, Y = accuracy) %>%
  smooth_obs_pred(fit, covar = c('err0')) %>% 
  mutate(err0 = covar1)
plt_dat %>% 
...
# intercepts
mcmc_hist(fit, pars = c('mu_loc'), binwidth = .005, 
  transform = list('mu_loc' = 'inv_logit')) + 
  my_theme + 
  geom_histogram(aes(x = inv_logit(rnorm(8e3, -.5, .5))), alpha = .2, binwidth = .005) +
...
mcmc_hist(fit, pars = c('sigma_loc'), binwidth = .005, 
  transform = list('sigma_loc' = 'inv_logit')) + 
  my_theme + 
...
mcmc_hist(fit, pars = c('beta_loc'), binwidth = .005, 
...
# reward differences
mu_diff <- get_diff(get_mu(draws, R_mu = 1, covar = TRUE), par = 'mu', R = 1)
sigma_diff <- get_diff(get_sigma(draws, R_sigma = 1, covar = TRUE), par = 'sigma', R = 1)
beta_diff <- get_diff(get_beta(draws, R_beta = 1, covar = TRUE), par = 'beta', R = 1)
...
# mcmc_hist(fit_mix, pars = c('delta_lambda[1]'), binwidth = .005, 
