...
functions {
  vector get_theta(int N, int[] id, vector time, vector x, vector mu0, vector sigma0, vector beta0, vector alpha0, vector delta_mu, vector delta_sigma) {
...
model {
  ...
  delta_mu_scale ~ normal(.5, .5);
  delta_sigma_scale ~ normal(.5, .5);
