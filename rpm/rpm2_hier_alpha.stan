...
functions {
  vector get_theta(int h, int g, int N, int J, int[] id, vector time, vector x, vector congr_rep, vector congr, matrix mu0, matrix sigma0, matrix beta0, vector alpha0, matrix delta_mu, matrix delta_sigma, matrix delta_beta, real delta_alpha) {
    ...
    for (n in 1:N) {
      ...
      alpha[n] = inv_logit(alpha0[id[n]] + delta_alpha * congr_rep[n]);  // to control for predictability bug...see e.g., /cse_simon/color/fig/obs.jpg @ PT < 100ms
...
transformed parameters {
  ...
  theta = get_theta(h, g, N, J, id, time, x, congr, mu0, sigma0, beta0, alpha0, delta_mu, delta_sigma, delta_beta, delta_alpha);
}
...
model {
  ...
  delta_alpha ~ normal(0, .5); // to control for predictability bug in repeat congruency...see /cse_simon/color/fig/obs.jpg
