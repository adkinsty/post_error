...
functions {
    vector get_theta(int h, int g, int N, vector time, vector x, vector congr, vector mu_loc, vector delta_mu, vector sigma, vector beta, real alpha) {
        ...
        for (n in 1:N) {
            ...
            mu[h]  = inv_logit(mu_loc[h] + delta_mu[h] * x[n]);
            mu[g]  = inv_logit(mu_loc[g] + delta_mu[g] * x[n]);
...
transformed parameters {
  ...
  theta = get_theta(h, g, N, time, x, congr, mu_loc, delta_mu, sigma, beta, alpha);
}
