...
plt_dat %>% 
  ggplot(aes(x = t, y = obs_mu, 
    ...
    colour = factor(phase), fill = factor(phase), alpha = as.character(err0), group = interaction(phase, err0))) +
  sw_geom + 
  scale_fill_manual("Phase", labels = c("c", "l"), values = WONG[c(3, 2)]) +
  scale_colour_manual("Phase", labels = c("c", "l"), values = WONG[c(3, 2)]) +
  my_theme + 
...
plt_dat %>% 
  ...
  my_theme + ggsave(sprintf('fig/%s_skew_pred.jpg', exp_name), units = 'in', height = 2, width = 3, dpi = 1000)
...
dat %>%
  mutate(phase = as.factor(phase), err0 = as.factor(err0)) %>%
  group_by(phase, err0) %>% 
  summarise(
  ...
  ggplot(
    aes(x = err0, y = mean, ymin = lower, ymax = upper, fill = phase, colour = phase)) +
