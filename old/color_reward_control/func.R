...
design_matrix <- function(x_names, data) {
  ...
  for (name in x_names) {
    ...
    if (str_detect(name, '\\*')) {
      ...
      for (tmp_name in names) {
        new_x <- new_x * pull(data, tmp_name)
      }

    } else {

      new_x <- pull(data, name)
...
format_input <- function(data,
  ...
  input$N <- nrow(data)
  input$y <- data$accuracy
  input$time <- data$time
  input$congr <- data$congr
  input$phase <- data$phase
  input$err0 <- data$err0

  input$R_mu <- R_mu
  input$R_sigma <- R_sigma
  input$R_beta <- R_beta

  input$x_mu <- design_matrix(cov_mu, data)
  input$x_sigma <- design_matrix(cov_sigma, data)
  input$x_beta <- design_matrix(cov_beta, data)
  input$x_alpha <- design_matrix(cov_alpha, data)
...
format_input_skew <- function(data,
  R_mu, R_sigma, R_lambda, R_beta, cov_mu, cov_sigma, cov_lambda, cov_beta, cov_alpha, gq, prior_only) {
  ...
  input$R_sigma <- R_sigma
  input$R_beta <- R_beta
  input$R_lambda <- R_lambda

  input$x_mu <- design_matrix(cov_mu, data)
  input$x_sigma <- design_matrix(cov_sigma, data)
  input$x_lambda <- design_matrix(cov_lambda, data)
  input$x_beta <- design_matrix(cov_beta, data)
  input$x_alpha <- design_matrix(cov_alpha, data)
...
get_beta <- function(draws, R_beta = 2, covar = FALSE) {
...
get_lambda <- function(draws, R_lambda = 2, covar = TRUE) {
  lambda_draws <- draws %>% subset_draws(variable = 'delta_lambda*', regex = TRUE)
  if (!covar) {
    lambda_draws <- lambda_draws %>%
      mutate_variables(`lambda[h]` = 1 / inv_logit_scaled(`delta_lambda[1,1]`))
    if (R_lambda == 2) {
      lambda_draws <- lambda_draws %>%
        mutate_variables(`lambda[g]` = 1 / inv_logit_scaled(`delta_lambda[1,2]`))
    }
  } else {
    lambda_draws <- lambda_draws %>%
      mutate_variables(`lambda[h,lo]` = 1 / inv_logit_scaled(`delta_lambda[1,1]` + `delta_lambda[2,1]` * -.5),
        `lambda[h,hi]` = 1 / inv_logit_scaled(`delta_lambda[1,1]` + `delta_lambda[2,1]` * .5))
    if (R_lambda == 2) {     
      lambda_draws <- lambda_draws %>% 
        mutate_variables(`lambda[g,lo]` = 1 / inv_logit_scaled(`delta_lambda[1,2]` + `delta_lambda[2,2]` * -.5),
          `lambda[g,hi]` = 1 / inv_logit_scaled(`delta_lambda[1,2]` + `delta_lambda[2,2]` * .5))
    }
  }
  return(lambda_draws)
}

get_mu <- function(draws, R_mu = 2, covar = TRUE) {
  ...
  if (!covar) {
    ...
    if (R_mu == 2) {
      mu_draws <- mu_draws %>%
        mutate_variables(`mu[g]` = inv_logit_scaled(`delta_mu[1,2]`))
    }
  } else {
    mu_draws <- mu_draws %>%
    ...
    if (R_mu == 2) {     
      mu_draws <- mu_draws %>% 
        mutate_variables(`mu[g,lo]` = inv_logit_scaled(`delta_mu[1,2]` + `delta_mu[2,2]` * -.5),
...
get_sigma <- function(draws, R_sigma = 2, covar = TRUE) {
  ...
  if (!covar) {
    ...
    if (R_sigma == 2) {
      sigma_draws <- sigma_draws %>%
        mutate_variables(`sigma[g]` = inv_logit_scaled(`delta_sigma[1,2]`))
    }
  } else {
    sigma_draws <- sigma_draws %>%
    ...
    if (R_sigma == 2) {     
      sigma_draws <- sigma_draws %>% 
        mutate_variables(`sigma[g,lo]` = inv_logit_scaled(`delta_sigma[1,2]` + `delta_sigma[2,2]` * -.5),
...
multiverse <- function(data = c(), cov_param = c('mu', 'sigma', 'beta'), R_param = c('sigma', 'beta')) {
  ...
  for (exp_name in all_exp) {
    ...
    exp_dat <- data %>% filter(exp == exp_name)
...
plot_mu <- function(draws, exp_name) {
  ...
  ggsave(plot = p, filename = path, units = 'in', height = 1.5, width = 2.25, dpi = 1000)

  return(p)
}

plot_prep <- function(draws, exp_name) {
  ...
  prep <- mus %>% 
    ...
    summarise(t = seq(0, 1, .001),
      ...
      dens.upper = dnorm(t, mu.upper, sigma.upper), 
      dens.lower = dnorm(t, mu.lower, sigma.lower))

  p <- prep %>% mutate(v = factor(v, c('lo', 'hi'))) %>%
    ggplot(aes(x = t*1000, y = dens, fill = r, colour = r, alpha = v, group = interaction(r, v))) + 
    geom_line() +
    scale_fill_manual('Response', values = WONG[c(4, 8)]) +
    scale_colour_manual('Response', values = WONG[c(4, 8)]) +
    scale_alpha_manual('Reward', values = c(.5, 1)) +
    scale_x_continuous('Time (ms)', breaks = seq(0, 1000, 100)) +  
    coord_cartesian(xlim = c(50, 700), ylim = c(0, 10)) + 
    my_theme +  
    theme(axis.line.y = element_blank(), 
      panel.grid.minor.y = element_blank(),
      panel.grid.major = element_blank())

  path <- sprintf('fig/%s_prep_dens.jpg', exp_name)
  ggsave(plot = p, filename = path, units = 'in', height = 1.5, width = 3, dpi = 1000)

  return(p)
}

plot_prep_skew <- function(draws, exp_name) {
  sigmas <- get_sigma(draws) %>% spread_draws(sigma[r, v])
  mus <- get_mu(draws) %>% spread_draws(mu[r, v])
  lambdas <- get_lambda(draws) %>% spread_draws(lambda[r, v])
  ...
  prep <- mus %>% 
    ...
    group_by(r, v) %>%
    summarise(t = seq(0, 1, .001),
      dens = dskew_normal(x = t, mu = mu, sigma = sigma, alpha = lambda), 
      dens.upper = dskew_normal(x = t, mu = mu.upper, sigma = sigma.upper, alpha = lambda.upper), 
      dens.lower = dskew_normal(x = t, mu = mu.lower, sigma = sigma.lower, alpha = lambda.lower))

  if (str_detect(exp_name, 'control')) {
    pal_id <- c(3, 2)
  } else {
    pal_id <- c(4, 8)
  }
  ...
  p <- prep %>% mutate(v = factor(v, c('lo', 'hi'))) %>%
    ggplot(aes(x = t*1000, y = dens, fill = r, colour = r, alpha = v, group = interaction(r, v))) + 
    geom_line() +
    scale_fill_manual('Response', values = WONG[pal_id]) +
    scale_colour_manual('Response', values = WONG[pal_id]) +
    scale_alpha_manual('Reward', values = c(.5, 1)) +
    scale_x_continuous('Time (ms)', breaks = seq(0, 1000, 100)) +  
    coord_cartesian(xlim = c(50, 700), ylim = c(0, 10)) + 
    my_theme +  
  ...
  path <- sprintf('fig/%s_skew_prep_dens.jpg', exp_name)
  ggsave(plot = p, filename = path, units = 'in', height = 1.5, width = 3, dpi = 1000)
...
#   p <- sigmas %>% 
#     spread_draws(sigma[r]) %>% 
#     ggplot(aes(x = sigma*1000, colour = r, fill = r)) + 
#     stat_halfeye(point_size = 1, slab_alpha = .5) +
#     scale_fill_manual('Response', values = WONG[c(4, 8)]) +
#     scale_colour_manual('Response', values = WONG[c(4, 8)]) +
#     scale_x_continuous(expression(paste('Preparation Noise (',sigma,')')), breaks = seq(0, 200, 50)) +  
...
#   path <- sprintf('fig/%s_sigma.jpg', exp_name)
#   ggsave(plot = p, filename = path, units = 'in', height = 1.25, width = 2.75, dpi = 1000)
...
rpm <- function(data,
  ...
  exp_name <- data$exp[1]
  fit_filename <- sprintf('fit/rpm_%s_mu_%s%s_sigma_%s%s_beta_%s%s_alpha_1%s.rds', 
    exp_name, R_mu, str_C(cov_mu), R_sigma, str_C(cov_sigma), R_beta, str_C(cov_beta), str_C(cov_alpha))
  
  loo_filename <- sprintf('loo/loo_rpm_%s_mu_%s%s_sigma_%s%s_beta_%s%s_alpha_1%s.rds', 
    exp_name, R_mu, str_C(cov_mu), R_sigma, str_C(cov_sigma), R_beta, str_C(cov_beta), str_C(cov_alpha))
...
rpm_skew <- function(data,
  R_mu = 2, R_sigma = 2, R_lambda = 2, R_beta = 1, 
  cov_mu = c(), cov_sigma = c(), cov_lambda = c(), cov_beta = c(), cov_alpha = c(), 
  gq = 1, prior_only = 0, re_run = FALSE,
  chains = 4, cores = 2, iter = 3000, warmup = 1000) {
  ...
  exp_name <- data$exp[1]
  fit_filename <- sprintf('fit/rpm_skew_%s_mu_%s%s_sigma_%s%s_lambda_%s%s_beta_%s%s_alpha_1%s.rds', 
    exp_name, R_mu, str_C(cov_mu), R_sigma, str_C(cov_sigma), R_lambda, str_C(cov_lambda), R_beta, str_C(cov_beta), str_C(cov_alpha))
  
  loo_filename <- sprintf('loo/loo_rpm_skew_%s_mu_%s%s_sigma_%s%s_lambda_%s%s_beta_%s%s_alpha_1%s.rds', 
    exp_name, R_mu, str_C(cov_mu), R_sigma, str_C(cov_sigma), R_lambda, str_C(cov_lambda),  R_beta, str_C(cov_beta), str_C(cov_alpha))
  ...
  if (re_run | !file.exists(fit_filename)) {
    ...
    if (str_detect(exp_name, 'control')) {
      stan_code <- stan_model('rpm_skew_ctrl.stan')
    } else {
      stan_code <- stan_model('rpm_skew.stan')
    }
    stan_dat <- format_input_skew(
      data, 
      R_mu, R_sigma, R_lambda, R_beta, 
      cov_mu, cov_sigma, cov_lambda, cov_beta, cov_alpha, 
      gq, prior_only)
  ...
  output <- data.frame() %>%
    ...
    mutate(
      exp = exp_name,
      R_mu, R_sigma, R_lambda, R_beta,
      cov_mu = str_C(cov_mu), cov_sigma = str_C(cov_sigma), cov_lambda = str_C(cov_lambda), 
      cov_beta = str_C(cov_beta), cov_alpha = str_C(cov_alpha),
      filename = fit_filename)
...
smooth_obs <- function(data, lb = 50, ub = 700) {
  ...
  if (str_detect(exp_name, 'control')) {
    tmp <- obs_pred %>% group_by(phase, err0)
  } else {
    tmp <- obs_pred %>% group_by(congr, err0)
  }
...
smooth_obs_pred <- function(data, fit, lb = 50, ub = 700) {
  # given dataset and model for a single experiment
  # run sliding window analysis on accuracy over time
  # return smoothed time-series of accuracy
...
sw_smooth <- function(x, y, lb = 50, ub = 700, width = 100, FUN = mean) {
