...
functions {
  vector get_theta(int N, vector time, vector phase, int R_mu, int R_sigma, int R_beta, matrix x_mu, matrix x_sigma, matrix x_beta, matrix x_alpha, matrix delta_mu,  matrix delta_sigma, matrix delta_beta, vector delta_alpha) {
    ...
    for (n in 1:N) {
      ...
      if (phase[n] > 0) {
        phi = normal_cdf(time[n], mu[n, 1], sigma[n, 1]); // prob. that habit is prepared
        psi[1] = (1 - phi) * alpha[n]; 
        psi[2] = phi * beta[n, 1]; 
      } else {
        phi = normal_cdf(time[n], mu[n, R_mu], sigma[n, R_sigma]); // prob. that goal is prepared
        psi[1] = (1 - phi) * alpha[n]; 
...
data {
  ...
  vector[N] phase; // location or color phase
...
transformed parameters {
  vector[N] theta;
  theta = get_theta(N, time, phase, R_mu, R_sigma, R_beta, x_mu, x_sigma, x_beta, x_alpha, delta_mu, delta_sigma, delta_beta, delta_alpha);
}
model {
