...
get_sigma <- function(draws, R_sigma = 2, covar = FALSE) {
...
plot_mu <- function(draws, exp_name) {
  ...
  p <- mus %>% 
    ...
    scale_y_continuous("Parameter", expand = c(.2, .2), breaks = 1:4, labels = c(expression(mu["g,c0"]), expression(mu["g,i0"]), expression(mu["h,c0"]), expression(mu["h,i0"]))) +
    scale_x_continuous("Time (ms)", breaks = seq(0, 500, 50)) +  
...
plot_prep <- function(draws, exp_name) {
  sigmas <- get_sigma(draws) %>% spread_draws(sigma[r])
...
smooth_obs_pred <- function(data, fit, lb = 50, ub = 700) {
  ...
  obs_pred <- data %>% 
    mutate(
      theta = apply(X = pred, FUN = median, MARGIN = 1),
