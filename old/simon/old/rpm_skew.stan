...
functions {
  vector get_theta(int N, vector time, vector congr, int R_mu, int R_sigma, int R_lambda, int R_beta, matrix x_mu,  matrix x_sigma, matrix x_lambda, matrix x_beta, matrix x_alpha, matrix delta_mu,  matrix delta_sigma, matrix delta_lambda, matrix delta_beta, vector delta_alpha) {
    ...
    for (n in 1:N) {
      ...
      phi[1] = skew_normal_cdf(time[n], xi[n, 1], omega[n, 1], lambda[n, 1]); // prob. that habit is prepared
      phi[2] = skew_normal_cdf(time[n], xi[n, R_mu], omega[n, R_sigma], lambda[n, R_lambda]); // prob. that goal is prepared
      neg_phi[1] = 1 - phi[1];
      neg_phi[2] = 1 - phi[2];
