...
functions {
  vector get_theta(int N, vector time, vector congr, int R_mu, int R_sigma, int R_lambda, int R_beta, matrix x_mu,  matrix x_sigma, matrix x_lambda, matrix x_beta, matrix x_alpha, matrix delta_mu,  matrix delta_sigma, matrix delta_lambda, matrix delta_beta, vector delta_alpha) {
    ...
    lambda = inv(inv_logit(x_lambda * delta_lambda));
    ...
    for (n in 1:N) {
      ...
      phi[1] = exp_mod_normal_cdf(time[n], mu[n, 1], sigma[n, 1], lambda[n, 1]); // prob. that habit is prepared
      phi[2] = exp_mod_normal_cdf(time[n], mu[n, R_mu], sigma[n, R_sigma], lambda[n, R_lambda]); // prob. that goal is prepared
      neg_phi[1] = 1 - phi[1];
      neg_phi[2] = 1 - phi[2];
...
transformed parameters {
  ...
  theta = get_theta(N, time, congr, R_mu, R_sigma, R_lambda, R_beta, x_mu, x_sigma, x_lambda, x_beta, x_alpha, delta_mu, delta_sigma, delta_lambda, delta_beta, delta_alpha);
}
model {
  ...
  if (R_lambda > 1) {
    delta_lambda[1, R_lambda] ~ normal(-2, 2);
  }
  if (R_beta > 1) {
