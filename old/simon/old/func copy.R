...
get_lambda <- function(draws, R_lambda = 2, covar = TRUE) {
  ...
  if (!covar) {
    lambda_draws <- lambda_draws %>%
      mutate_variables(`lambda[h]` = 1 / `delta_lambda[1,1]`)
    if (R_lambda == 2) {
      lambda_draws <- lambda_draws %>%
        mutate_variables(`lambda[g]` = 1 / `delta_lambda[1,2]`)
    }
  } else {
    lambda_draws <- lambda_draws %>%
      mutate_variables(`lambda[h,lo]` = 1 / (`delta_lambda[1,1]` + `delta_lambda[2,1]` * -.5),
        `lambda[h,hi]` = 1 / (`delta_lambda[1,1]` + `delta_lambda[2,1]` * .5))
    if (R_lambda == 2) {     
      lambda_draws <- lambda_draws %>% 
        mutate_variables(`lambda[g,lo]` = 1 / (`delta_lambda[1,2]` + `delta_lambda[2,2]` * -.5),
          `lambda[g,hi]` = 1 / (`delta_lambda[1,2]` + `delta_lambda[2,2]` * .5))
    }
  }
  return(lambda_draws)
}
...
smooth_obs <- function(data, lb = 50, ub = 700) {
  ...
  tmp <- obs_pred %>% group_by(congr, err0, stim_rep)
