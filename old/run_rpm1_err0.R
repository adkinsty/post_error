# library(tidyverse) # Wickham et al., (2019). Welcome to the tidyverse. Journal of Open Source Software, 4(43), 1686, https://doi.org/10.21105/joss.01686
# library(shinystan)

#path <- '/Users/adkinsty/Dropbox (University of Michigan)/LeeLab/Experiments/Exp_files/'
path <- '/home/ec2-user'
exp_name <- 'memory'
proj_name <- 'post_error'
mod_name <- 'rpm1_hier'
covar_name <- 'err0'

setwd(sprintf('%s/forced_response/post_error/%s/', path, exp_name))
source('../../../forced_response/func.R')

dat <- read_csv("dat.csv")

results <- fit_rpm(dat = dat, exp_name = exp_name, mod_name = mod_name, covar = covar_name, re_run = TRUE)

# fit_hier <- read_rds(sprintf('fit/%s_rpm1_hier_full_mu_1err0_sigma_1err0_beta_1_alpha_1.rds',exp_name))
# loo_hier <- read_rds(sprintf('loo/loo_%s_rpm1_hier_full_mu_1err0_sigma_1err0_beta_1_alpha_1.rds',exp_name))
# shinystan::launch_shinystan(fit_hier)

# rnorm(1e4, -2, .5) %>%  inv_logit() %>% qplot() + xlim(0, 1)
# rnorm(1e4, -1, .5) %>% inv_logit() %>% qplot() + xlim(0, 1)
# rnorm(1e4, -.5, .5) %>% inv_logit() %>% qplot() + xlim(0, 1)
# rnorm(1e4, 0, .5) %>% inv_logit() %>% qplot() + xlim(0, 1)
# rnorm(1e4, 2, .5) %>% inv_logit() %>% qplot() + xlim(0, 1)

# results <- rpm(
#   dat = dat, 
#   mod_type = 'rpm1',
#   re_run = TRUE, 
#   R_mu = 1, 
#   R_sigma = 1, 
#   R_beta = 1,
#   cov_mu = c("err0"), 
#   cov_sigma = c("err0"), 
#   cov_beta = c("err0"))

# # follow up modeling of post-error data

# dat_err0 <- dat %>% filter(err0 == .5)

# results <- rpm(
#   dat = dat_err0, 
#   exp_name = 'memory_err0',
#   mod_type = 'rpm1',
#   re_run = TRUE)

# results <- rpm(
#   dat = dat_err0, 
#   mod_type = 'rpm_mix',
#   exp_name = 'memory_err0',
#   re_run = TRUE, 
#   R_mu = 2, 
#   R_sigma = 2)

# exp_name <- 'memory_err0'
# fit1 <- read_rds(sprintf('fit/rpm1_%s_mu_1_sigma_1_beta_1_alpha_1.rds',exp_name))
# fit_mix <- read_rds(sprintf('fit/rpm_mix_%s_mu_2_sigma_2_beta_1_alpha_1_lambda_1.rds',exp_name))

# shinystan::launch_shinystan(fit_mix)
