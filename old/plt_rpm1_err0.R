...
mod_name <- 'rpm1_hier_mu_%s_sigma_%s_beta_err0'
...
p <- plot_prep1_cov(draws, exp_name) +  
  scale_x_continuous('Time (ms)', breaks = seq(0, 1000, 100), labels = as.character(seq(0, 2000, 200))) +  
  ggsave(filename = sprintf('fig/%s_prep_dens1_cov.jpg', mod_name), units = 'in', height = 1.5, width = 3.5, dpi = 1000)

get_mu(draws, R_mu = 1, covar = FALSE) %>% spread_draws(mu) %>% median_qi()
get_sigma(draws, R_sigma = 1, covar = FALSE) %>% spread_draws(sigma) %>% median_qi()
get_beta(draws, R_beta = 1, covar = FALSE) %>% spread_draws(beta) %>% median_qi()

# group intercepts

p <- mcmc_hist(fit, pars = c('mu_loc'), binwidth = .005, 
  transform = list('mu_loc' = 'inv_logit2')) + 
  my_theme + 
  hist_theme +
  geom_histogram(aes(x = 2*inv_logit(rnorm(8e3, -.5, .5))), alpha = .2, binwidth = .005) +
...
p <- mcmc_hist(fit, pars = c('sigma_loc'), binwidth = .005, 
  transform = list('sigma_loc' = 'inv_logit2')) + 
  my_theme + 
  hist_theme + 
  geom_histogram(aes(x = 2*inv_logit(rnorm(8e3, -2, .5))), alpha = .2, binwidth = .005) +
...
mu_diff <- get_diff(mus, par = 'mu', R = 1) %>% 
  mutate_variables(mu_diff = mu_diff*2) %>% 
  subset_draws(variable = 'mu_diff')
mu_diff %>% median_qi()
mu_diff %>% p_direction()

sigma_diff <- get_diff(sigmas, par = 'sigma', R = 1) %>% 
  mutate_variables(sigma_diff = sigma_diff*2) %>% 
  subset_draws(variable = 'sigma_diff')
...
p <- mcmc_hist(mu_diff, pars = c('mu_diff'), binwidth = .005, 
  transform = list('mu_diff' = 'x2')) + 
  my_theme + 
  hist_theme + 
  geom_histogram(aes(x = 2*inv_logit(prior_start_mu + prior_slope * .5) - 2*inv_logit(prior_start_mu + prior_slope * -.5)), alpha = .2, binwidth = .005) +
...
p <- mcmc_hist(sigma_diff, pars = c('sigma_diff'), binwidth = .005, 
  transform = list('sigma_diff' = 'x2')) + 
  my_theme + 
  hist_theme + 
  geom_histogram(aes(x = 2*inv_logit(prior_start_sig + prior_slope * .5) - 2*inv_logit(prior_start_sig + prior_slope * -.5)), alpha = .2, binwidth = .005) +
...
facet_labels <- ifelse(covar_name == 'rew', c('$1', '$2'), ifelse(covar_name == 'congr0', c('After incongruent', 'After congruent'), c('After correct', 'After incorrect')))
...
p <- plt_dat %>% 
  ...
  scale_colour_manual(values = WONG[c(4, 7)]) +
  scale_fill_manual(values = WONG[c(4, 7)]) +
  facet_wrap(key_stim_rep ~ ., labeller = labeller(key_stim_rep = c('TRUE' = 'Goal is previous key', 'FALSE' = 'Goal is not previous key'))) +
